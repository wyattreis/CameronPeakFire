...
spice <- read.csv("AWS/SPICE/BACKUP.CSV") %>% 
  ...
  mutate(rolling_sd = rollmean(sd_sonic, 3, fill = NA), site = "UF_c")
```
...
# Daily Wx sonic snow depth data
daily_sonic <- all_hr %>% 
  select(c(1,2,10)) %>% 
...
AWS_daily <- list(daily_cam, pit_density, daily_sonic, JW_SWE, spice) %>% 
  ...
  mutate(sd = coalesce(depth_avg, pit_SD, depth, rolling_sd),
         ...
         bulk_density = coalesce(bulk_density.x, bulk_density.y)) %>% #coalesce to fill gaps in camera and sonic time series with pit 
  group_by(site) %>% 
